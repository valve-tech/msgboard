...
/// a harmed player. A dispute-LOCK path (freezing bond while a dispute is open) is a future slice; it is
/// NOT built here, so the whole `total` is always free to withdraw or slash.
///
...
contract OperatorBond is ReentrancyGuard {
    ...
    function bondOf(address operator, address token) external view returns (uint256 total) {
        return bonds[_key(operator, token)].total;
    }
    ...
    function withdrawBond(address token, uint256 amount) external nonReentrant {
        ...
        if (amount > b.total) revert InsufficientBond();
        b.total -= amount;
