...
contract HoldemShareDisputeTest is Test {
    ...
    /// `_seatOf(msg.sender)` — a STRANGER, never seated at this table at all, relays the DEMANDED
    /// seat's genuinely valid share + DLEQ proof (generated by the REAL off-chain prover via
    /// vm.ffi, exactly like `test_correctShareResolvesDispute`) and the dispute clears exactly as
    /// if the demand seat had submitted it directly. The stranger can only ever land the TRUE
    /// demand seat's own truthful share — the proof is checked against
    /// `_deckKey[tableId][DEMAND_SEAT]`, a secret only that seat's holder can produce a passing
    /// witness for — so it can never forge one, redirect one to a different seat, or move any
    /// funds (this function only clears the dispute; it never pays out).
    function test_strangerRelaysValidShare_clearsDispute() public {
        (bytes32 tableId, Gen memory g) = _setupDisputed(4 ether);
        address stranger = address(0xFEED); // never a seat at this table
        vm.prank(stranger);
        zk.respondWithShare(tableId, g.deck, g.share, g.proof);
        assertEq(uint8(zk.status(tableId)), uint8(ChannelTableBase.Status.Live), "dispute cleared by the stranger's relay");
    }

    /// A wrong-pk (forged) proof reverts regardless of who calls it — the caller's identity
    /// (demand seat, another seat, or a total stranger) is irrelevant; only a passing DLEQ proof
    /// against the demand seat's REGISTERED key ever clears the dispute.
    function test_forgedShareReverts_regardlessOfCaller() public {
        (bytes32 tableId, Gen memory g) = _setupDisputed(4 ether);
        g.proof[4] ^= 1; // corrupt the response scalar z
