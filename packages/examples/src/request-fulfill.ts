...
type Request = { action: string; params: string; issuedAt: number; nonce: string }

/** The wire envelope posted to the board: the request, who signed it, and the signature. */
type Envelope = { request: Request; signer: Address; signature: Hex }

/** Canonical bytes that get signed — deterministic key order so signer and verifier agree. */
const canonical = (request: Request): string =>
...
const sign = async (request: Request, account: ReturnType<typeof privateKeyToAccount>): Promise<Envelope> => ({
  request,
...
const encode = (envelope: Envelope): Hex => toHex(JSON.stringify(envelope))

/** Decodes a board message's hex `data` back into an envelope, or null if it isn't one. */
const decode = (data: Hex): Envelope | null => {
  try {
...
/** Recovers the signer and confirms it authorized exactly this request. */
const isAuthentic = (envelope: Envelope): Promise<boolean> =>
  verifyMessage({ address: envelope.signer, message: canonical(envelope.request), signature: envelope.signature })

const fulfill = (envelope: Envelope): void => {
