 ...
 */
const makeCollector = (owners: readonly Address[], threshold: number) => {
  ...
  const offer = async (partial: Partial): Promise<{ accepted: boolean; reason?: string; complete?: Partial[] }> => {
    ...
    const authentic = await verifyMessage({ address: partial.signer, message: partial.payload, signature: partial.signature })
...
const sign = async (payload: string, account: ReturnType<typeof privateKeyToAccount>): Promise<Partial> => ({
  payload,
