...
| `randomness-game.ts` | `npm run randomness-game --workspace=packages/examples` | offline mock / live read (943) | a provably-fair dice game whose seed comes from the [gibsfinance/random](https://github.com/gibsfinance/random) commit-reveal contract; offline mocks the lifecycle, `RANDOM_RPC`+`RANDOM_KEY` read a real on-chain seed |
...
| `zk-msgboard.ts` | `npm run zk-msgboard --workspace=packages/examples` | offline demo / watcher / poster | a Semaphore-style **zk-filtered archive**: members prove group membership in zero knowledge to post; a watcher verifies every proof (real snarkjs) and archives only the subset that "makes it through" — provably in-group, without revealing which member |
...
**ZK-filtered archive** — a port of the [zk-message-board](https://github.com/nulven/zk-message-board)
concept onto MsgBoard. Members of a committed group (a Merkle set of Poseidon identity
commitments) prove — in zero knowledge — that they are *one of* the allowed members, without
revealing *which* one, and attach a nullifier so the archive can rate-limit them. The Groth16
proof + public signals ride inside the message `data`. A watcher verifies every proof and passes
through only the messages that carry a valid membership proof for the known group with an unused
nullifier. That filtered subset — the messages that "made it through the zk msgboard" — **is** the
archive: a provably-gated view of board traffic that is attributable-to-the-group-but-not-the-individual.

The example has two roles, matching the framing "post to a single category OR run a full watcher
that filters against it":

- **post** — produce a membership proof + nullifier bound to the exact message, then post it to one
  category (`ZK_MODE=post`, requires `MSGBOARD_RPC` and the built artifacts).
- **watch** — a `Relayer` whose **sink** really verifies each proof (`snarkjs groth16.verify`) and
  archives only the survivors (default when `MSGBOARD_RPC` is set). The sink runs in `observe` mode
  and takes no on-chain action — it is a read-only filter.

Modes:

- No `MSGBOARD_RPC` (default): an offline walkthrough of the four filter gates against committed
  fixtures — two members admitted; an outsider (valid proof for a *different* group) rejected on
  root mismatch; a double-post rejected on nullifier reuse; a proof lifted onto different content
  rejected on signal binding; a tampered proof rejected on verification. If you have run `zk:build`
  it also generates a fresh proof live.
- `MSGBOARD_RPC` set: runs the live watcher over the `zkarchive` category (`ZK_CATEGORY` /
  `ZK_GROUP_ROOT` to override), or `ZK_MODE=post` to prove-and-post.

**The ZK setup.** The circuit is `circuits/membership.circom` (Poseidon commitment + Merkle
inclusion + nullifier, a compact Semaphore-v2 re-implementation). `npm run zk:build` compiles it
with `circom` and runs a **deterministic DEV/TEST-ONLY** trusted setup via `snarkjs ... beacon`
(a fixed public beacon, *not* `contribute`, so the verification key is reproducible). The committed
`zk/verification_key.json` + `zk/fixtures/*.json` are enough to verify offline; the heavy artifacts
(ptau/zkey/wasm/r1cs under `zk/build/`) are gitignored and regenerated by `zk:build`.

> ⚠️ The trusted setup is DEV/TEST-ONLY: the beacon (and thus the toxic waste) is public, so anyone
> could forge proofs. A real deployment needs a genuine multi-party ceremony and the audited
> Semaphore circuits. See [`docs/zk-msgboard.md`](../../docs/zk-msgboard.md) for what is deferred.
