...
/**
 * Returns the chain-scoped storage key prefix.
 *
 * Uses `chainId:rpcUrl` as the unique identifier so that each chain's UI state is
 * stored independently. Unlike the Svelte version (which read `chain.chain?.id` /
 * `chain.rpcUrl` reactively), this takes the values as explicit args — callers pass
 * `useChainStore.getState()` derivations (or the `useChainScope()` hook).
 */
export function getScope(chainId: number | undefined, rpcUrl: string | undefined): string {
  return `${chainId ?? 0}:${rpcUrl ?? ''}`
}
