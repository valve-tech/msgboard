...
/**
 * Off-main-thread tripwire (mirrors packages/cosign-web's / packages/ui's identical guard —
 * PoW must NEVER run on the main thread).
 *
 * This is a cheap source-level regression guard so nobody inlines the grind onto the render
 * thread. It asserts the seam's default worker factory spawns a dedicated ES-module `Worker`
 * (so the grind is bundled OFF the main thread as its own chunk — confirmed by `npm run build`
 * emitting a separate `pow-worker-*.js` chunk), and that the worker uses the dedicated-worker
 * global scope (not a service worker, not the main `window`).
 */
