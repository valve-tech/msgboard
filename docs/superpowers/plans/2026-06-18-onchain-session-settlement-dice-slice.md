...
- **⚠️ HouseChannel MUST be redeployed before real funds.** The live `0x5787…46e` predates the funds-safety hardening (commit `a5af087`: `gameId` binding at settle + `disputeFromOpen` refund floor). Task 5 deploys the patched contract; the NEW address replaces `0x5787…46e` everywhere it appears in this plan (the `makeSettleDomain` `verifyingContract`, `config.ts`, and the indexer's `ponder.config.ts` address + `startBlock`). Do not wire the frontend or indexer to the old address.
