...
- [ ] **Step 3: Implement** — replace the ephemeral house signer + `PLACEHOLDER_VERIFIER` with: a board `CoSignTransport` (post to `…:table:<chain>:<tableId>`, poll for the house's halves; grind in the Worker), `verifyingContract = deployment.houseChannel`, and `runPlayerSide`. Keep the existing return shape the screens use.
  - **SECURITY (carried from the Task 3 review — anti-house-bias): the player MUST bind `proof.clientSeed` to its OWN chosen client seed.** Task 3's `runPlayerSide` recomputes the outcome but takes `clientSeed` from the house-supplied `RoundProof` without checking it. In real split play the player picks its `clientSeed` (its entropy contribution), commits/announces it at OPEN, and on each round asserts `proof.clientSeed === <its own value>` before co-signing — otherwise the house controls both seeds and can grind the result. Add a failing test for "player rejects a round whose `clientSeed` ≠ its committed value" and make it pass. This is required for the player's fairness guarantee.
- [ ] **Step 4: Run** → PASS.
